    ...
    >
      <v-card>
        ...
        <v-card-title icon="mdi-index">
          ...
          ></v-text-field>
          <v-spacer v-show="!$vuetify.breakpoint.mobile"></v-spacer>
          <add-button @add="dialog=true,titleClick='addTrainingProgram'" v-if="checkRole('Create')"
                      v-shortkey="['+']"
                      @shortkey.native="dialog=true,titleClick='addTrainingProgram'"
                      v-show="!$vuetify.breakpoint.mobile"></add-button>
          <raise-button @add="dialog=true,titleClick='addTrainingProgram'" v-if="checkRole('Create')"
      ...
      >
        <v-card>
          ...
          <v-card-title>
            ...
            <v-icon v-if="titleClick==='addTrainingProgram'" large color="green darken-2"
                    style="font-size: 50px !important;">library_add
            </v-icon>
            <v-icon v-if="titleClick==='updateTrainingProgram'" large color="green darken-2"
...
import {Web_TrainingProgramReact} from "../../collections/trainingProgram"
...
export default {
  meteor: {
    reactData() {
      let vm = this;
      if (Meteor.userId()) {
        Web_TrainingProgramReact.find({}).fetch();
  ...
  data() {
    return {
      ...
      nameRules: [
        v => !!v || 'TrainingProgram Name is required',
      ],
  ...
  methods: {
    ...
    onUpload(num) {
      let vm = this;
      const storageRef = firebase.storage().ref("trainingProgram/" + moment().format("YYYYMMDD") + "/" + moment().format("YYYYMMDDHHmmss") + this.fileName).put(this.selectedFile);
    ...
    onUploadList(selectedFile, fileName) {
      let vm = this;
      const storageRef = firebase.storage().ref("TrainingProgram/" + moment().format("YYYYMMDD") + "/" + moment().format("YYYYMMDDHHmmss") + fileName).put(selectedFile);
      storageRef.on(`state_changed`, snapshot => {
            this.uploadValue = (snapshot.bytesTransferred / snapshot.totalBytes) * 100;
          }, error => {
            console.log(error.message)
    ...
    fetchDataTable: _.debounce(function (val, skip, limit) {
      ...
      return new Promise((resolve, reject) => {
        Meteor.call("web_fetchTrainingProgram", {
          q: val,
          filter: this.filter,
    ...
    handleSubmit() {
      ...
      if (vm.$refs.formData.validate()) {
        ...
        if (vm.dataObj._id === "") {
          return new Promise((resolve, reject) => {
            Meteor.call("web_insertTrainingProgram", vm.dataObj, Constants.secret, (err, result) => {
              if (!err) {
                this.$message({
          ...
          return new Promise((resolve, reject) => {
            Meteor.call("web_updateTrainingProgram", vm.dataObj._id, vm.dataObj, Constants.secret, (err, result) => {
              if (!err) {
                this.$message({
                  message: this.$t('successNotification'),
                  showClose: true,
    ...
    handleUpdate(doc) {
      ...
      vm.titleClick = "updateTrainingProgram";
      vm.dialog = true;
      Meteor.setTimeout(function () {
    ...
    handleRemove(row) {
      ...
      vm.$confirm(this.$t('messageWarning'), this.$t('warning'), {
        ...
        Meteor.call("web_removeTrainingProgram", row, Constants.secret, (err, result) => {
          if (!err) {
            vm.$message({
  ...
  created() {
    ...
    Meteor.subscribe('web_trainingProgramReact');
