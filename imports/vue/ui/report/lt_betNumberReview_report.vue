<!--suppress ALL -->
<template>
    <div class="betNumberReview-report">
        <a4>
                    ...
                    <v-row type="flex" class="row-bg" justify="center">
                        <v-col cols="12">
                            <v-card
                                    ...
                                    max-width="100%"
                            >
                                <v-card-title>{{$t('betNumberReviewTitle')}}
                                    <v-spacer></v-spacer>
                                ...
                                <v-card-text>
                                    <v-row>
                                        ...
                                        <v-col cols="12" md="4" sm="4" xs="4">
                                            <v-autocomplete v-model="params.rankId"
                                                            :items="rankOptionList"
                                                            outlined
                                                            dense
                                                            item-text="label"
                                                            item-value="value"
            ...
            <v-row type="flex" class="row-bg" justify="center">
                ...
                <v-col cols="12" md="12" sm="12" style="text-align: center !important;">
                        <span style="font-family: 'Khmer OS Muol'; font-size: 15px;">{{$t('betNumberReviewTitle')}} -{{agentName}}</span><br>
                        <span style="font-family: 'Khmer OS Muol'; font-size: 15px;">ប្រចាំថ្ងៃទី៖ {{formatDate(params.date)}} {{$t("time")}}-{{$t(params.time)}}</span>
                </v-col>
        ...
        methods: {
          printReport() {
            ...
            queryRankOption(q) {
                let vm = this;
                return new Promise((resolve, reject) => {
                    Meteor.call("lt_fetchRankOption", q, Constants.secret, vm.$store.state.branchId, (err, result) => {
                        if (result) {
                            vm.rankOptionList = result;
                            resolve(result);
                        } else {
                            reject(err.message);
                        }
            ...
            handleRun() {
                ...
                if (this.$refs.formReport.validate()) {
                    Meteor.call('base_fetchBetNumberReviewReport', vm.params, Constants.secret, vm.$i18n.locale, (err, result) => {
                        if (result) {
