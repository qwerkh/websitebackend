    ...
    <div class="dailyByDay-report">
        <a4>
                    ...
                    <v-row type="flex" class="row-bg" justify="center">
                        <v-col cols="12">
                            <v-card
                                    ...
                                    max-width="100%"
                            >
                                <v-card-title>{{$t('dailyByDayTitle')}}
                                    <v-spacer></v-spacer>
            ...
            <v-row type="flex" class="row-bg" justify="center">
                ...
                <v-col cols="12" md="12" sm="12" style="text-align: center !important;">
                        <span style="font-family: 'Khmer OS Muol'; font-size: 15px;">{{$t('dailyByDayTitle')}} -{{agentName}}</span><br>
                        <span style="font-family: 'Khmer OS Muol'; font-size: 15px;">ប្រចាំថ្ងៃទី៖ {{formatDate(params.dateRange[0])}}-{{formatDate(params.dateRange[1])}}</span>
                </v-col>
    ...
    export default {
        ...
        methods: {
          printReport() {
            ...
            handleRun() {
                let vm = this;
                console.log(vm.$store.state.language);
                vm.loading = true;
                if (this.$refs.formReport.validate()) {
                    Meteor.call('base_fetchDailyByDayReport', vm.params, Constants.secret, vm.$i18n.locale, (err, result) => {
                        if (result) {
