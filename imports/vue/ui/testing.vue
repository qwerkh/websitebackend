        ...
        >
            <v-card>
                ...
                <v-card-title icon="mdi-index">
                    ...
                    ></v-text-field>
                    <v-spacer></v-spacer>
                    <add-button @add="dialog=true,titleClick='addTesting'" v-if="checkRole('Create')" v-shortkey="['+']"
                                @shortkey.native="dialog=true,titleClick='addTesting'"
                                v-show="!$vuetify.breakpoint.xsOnly"></add-button>
                    <raise-button @add="dialog=true,titleClick='addTesting'" v-if="checkRole('Create')"
            ...
            >
                <v-card>
                    <v-card-title>
                        ...
                        <v-icon v-if="titleClick==='addTesting'" large color="green darken-2"
                                style="font-size: 50px !important;">library_add
                        </v-icon>
                        <v-icon v-if="titleClick==='updateTesting'" large color="green darken-2"
        ...
        methods: {
            ...
            handleSubmit() {
                ...
                if (vm.$refs.formData.validate()) {
                    ...
                    if (vm.dataObj._id === "") {
                        return new Promise((resolve, reject) => {
                            Meteor.call("base_insertTesting", vm.dataObj,Constants.secret, (err, result) => {
                                if (!err) {
                                    this.$message({
                        ...
                        return new Promise((resolve, reject) => {
                            Meteor.call("base_updateTesting", vm.dataObj._id, vm.dataObj, Constants.secret, (err, result) => {
                                if (!err) {
                                    this.$message({
                                        message: this.$t('successNotification'),
                                        showClose: true,
            ...
            handleRemove(row) {
                ...
                vm.$confirm(this.$t('messageWarning'), this.$t('warning'), {
                    ...
                    Meteor.call("base_removeTesting", row,Constants.secret,(err, result) => {
                        if (!err) {
                            vm.$message({
