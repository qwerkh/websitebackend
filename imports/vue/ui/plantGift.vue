    ...
    >
      <v-card>
        ...
        <v-card-title icon="mdi-index">
          ...
          ></v-text-field>
          <v-spacer v-show="!$vuetify.breakpoint.mobile"></v-spacer>
          <add-button @add="dialog=true,titleClick='addPlantGift'" v-if="checkRole('Create')"
                      v-shortkey="['+']"
                      @shortkey.native="dialog=true,titleClick='addPlantGift'"
                      v-show="!$vuetify.breakpoint.mobile"></add-button>
          <raise-button @add="dialog=true,titleClick='addPlantGift'" v-if="checkRole('Create')"
      ...
      >
        <v-card>
          ...
          <v-card-title>
            ...
            <v-icon v-if="titleClick==='addPlantGift'" large color="green darken-2"
                    style="font-size: 50px !important;">library_add
            </v-icon>
            <v-icon v-if="titleClick==='updatePlantGift'" large color="green darken-2"
...
import {Web_PlantGiftReact} from "../../collections/plantGift"
...
export default {
  ...
  data() {
    return {
      ...
      fileNameList: "",
      newUrlList: [],
      pagePlantGiftList: Constants.pagePlantGiftList,
      dataObj: {
        _id: "",
        branchId: "",
        order: 1,
      ...
      nameRules: [
        v => !!v || 'PlantGift Name is required',
      ],
      phoneNumber: [
  ...
  methods: {
    ...
    onUpload(num) {
      let vm = this;
      const storageRef = firebase.storage().ref("plantGift/" + moment().format("YYYYMMDD") + "/" + moment().format("YYYYMMDDHHmmss") + this.fileName).put(this.selectedFile);
    ...
    onUploadList(selectedFile, fileName) {
      let vm = this;
      const storageRef = firebase.storage().ref("plantGift/" + moment().format("YYYYMMDD") + "/" + moment().format("YYYYMMDDHHmmss") + fileName).put(selectedFile);
      storageRef.on(`state_changed`, snapshot => {
            this.uploadValue = (snapshot.bytesTransferred / snapshot.totalBytes) * 100;
          }, error => {
            console.log(error.message)
    ...
    fetchDataTable: _.debounce(function (val, skip, limit) {
      ...
      return new Promise((resolve, reject) => {
        Meteor.call("web_fetchPlantGift", {
          q: val,
          filter: this.filter,
      ...
      if (vm.$refs.formData.validate()) {
        ...
        if (vm.dataObj._id === "") {
          return new Promise((resolve, reject) => {
            ...
            Meteor.call("web_insertPlantGift", vm.dataObj, Constants.secret, (err, result) => {
              if (!err) {
                this.$message({
          ...
          return new Promise((resolve, reject) => {
            Meteor.call("web_updatePlantGift", vm.dataObj._id, vm.dataObj, Constants.secret, (err, result) => {
              if (!err) {
                this.$message({
                  message: this.$t('successNotification'),
                  showClose: true,
    ...
    handleRemove(row) {
      ...
      vm.$confirm(this.$t('messageWarning'), this.$t('warning'), {
        ...
        Meteor.call("web_removePlantGift", row, Constants.secret, (err, result) => {
          if (!err) {
            vm.$message({
  ...
  created() {
    let vm = this;
    vm.fetchDataTable();
    Meteor.subscribe('web_plantGiftReact');
