    ...
    >
      <v-card>
        ...
        <v-card-title icon="mdi-index">
          ...
          ></v-text-field>
          <v-spacer v-show="!$vuetify.breakpoint.mobile"></v-spacer>
          <add-button @add="dialog=true,titleClick='addClient'" v-if="checkRole('Create')"
                      v-shortkey="['+']"
                      @shortkey.native="dialog=true,titleClick='addClient'"
                      v-show="!$vuetify.breakpoint.mobile"></add-button>
          <raise-button @add="dialog=true,titleClick='addClient'" v-if="checkRole('Create')"
      ...
      >
        <v-card>
          <v-card-title>
            ...
            <v-icon v-if="titleClick==='addClient'" large color="green darken-2"
                    style="font-size: 50px !important;">library_add
            </v-icon>
            <v-icon v-if="titleClick==='updateClient'" large color="green darken-2"
...
import {Web_ClientReact} from "../../collections/client"
...
export default {
  ...
  methods: {
    ...
    onUpload(num) {
      let vm = this;
      const storageRef = firebase.storage().ref("client/" + moment().format("YYYYMMDDHHmmss") + this.fileName).put(this.selectedFile);
      ...
      if (vm.$refs.formData.validate()) {
        ...
        if (vm.dataObj._id === "") {
          return new Promise((resolve, reject) => {
            ...
            Meteor.call("web_insertClient", vm.dataObj, Constants.secret, (err, result) => {
              if (!err) {
                this.$message({
          ...
          return new Promise((resolve, reject) => {
            Meteor.call("web_updateClient", vm.dataObj._id, vm.dataObj, Constants.secret, (err, result) => {
              if (!err) {
                this.$message({
                  message: this.$t('successNotification'),
                  showClose: true,
    ...
    handleRemove(row) {
      ...
      vm.$confirm(this.$t('messageWarning'), this.$t('warning'), {
        ...
        Meteor.call("web_removeClient", row, Constants.secret, (err, result) => {
          if (!err) {
            vm.$message({
  ...
  created() {
    let vm = this;
    vm.fetchDataTable();
    Meteor.subscribe('web_clientReact');
